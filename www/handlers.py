...
from common.web.apis import APIValueError, APIError, Page
from common.web.coroweb import get, post
from model.models import User, Blog, next_id
...
def signout(request):
    ...
    # # user2cookie(user, 0)
    # r = web.Response()
    # r.set_cookie(COOKIE_NAME, user2cookie(user, 0), max_age=0, httponly=True)
    # user.passwd = '******'
...
async def api_get_users(*, page='1'):
    ...
    # 指定page_size=2 即每页2条
    p = Page(num, page_index, page_size=2)
    if num == 0:
...
async def api_register_user(*, email, name, passwd):
    ...
    user = User(id=uid, name=name.strip(), email=email, passwd=hashlib.sha1(sha1_passwd.encode('utf-8')).hexdigest(),
                image='https://gravatar.com/avatar/%s?d=mm&s=120' % hashlib.md5(email.encode('utf-8')).hexdigest())
    # 保存
    await user.save()
...
async def api_update_blog(id, request, *, name, summary, content):
    logging.debug('api_update_blog.__user__.id %s' % request.__user__.id)
    logging.debug('api_update_blog.summary %s' % summary)
    if not name or not name.strip():
        raise APIValueError(name, '标题不能为空')
    if not summary or not summary.strip():
        raise APIValueError(summary, 'summary不能为空')
    ...
    logging.debug('api_update_blog.blog to update()')
    await blog.update()
    return blog
...
async def api_delete_blog(id, request):
    logging.debug('api_delete_blog.__user__.id %s' % request.__user__.id)
    blog = await Blog.find(id)
    await blog.remove()
    return blog
